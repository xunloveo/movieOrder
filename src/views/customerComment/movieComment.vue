<template>
	...
	<div class="customer-comment">
		...
		<div class="add-comment">
			...
			<div class="commit">
				<el-button type="success" @click='addMovieComment'>提交评论</el-button>
			</div>
		</div>
		<div class="comment-list">
			...
			<div v-show='commentListData.length==0 || noComment' style='text-align: center; margin: 20px 0;'>暂无评论！</div>		
		</div>		
...
<script>
	import {QueryWebComment, AddMovieComment} from "@/api/api"
	...
	export default {
		...
		methods: {
			...
			getCommentList() {			
				...
				QueryWebComment(params).then((res) => {
			...
			/**
			 * [addMovieComment description] 添加影评
			 */
			addMovieComment() {
	          ...
	          this.$router.push('/userlogin');
					...
					AddMovieComment(params).then((res) => {
						if(res.data.data){
							this.$message({
								type: 'success',
		...
		mounted() {
			console.log('mmmm', this.$route.query.id);
